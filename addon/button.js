...
function initButton(sfHost, inInspector) {
  ...
  function addFlowScrollability(popupEl) {
    ...
    if (currentUrl.includes("builder_platform_interaction")) {
      ...
      // Add Flow Scanner button
      addFlowScannerButton(headerFlow, currentUrl);
    }
  }

  function addFlowScannerButton(headerFlow, currentUrl) {
    // Create flow scanner button
    const flowScannerBtn = document.createElement("button");
    flowScannerBtn.id = "flow-scanner-btn";
    flowScannerBtn.textContent = "🔍 Flow Scanner";
    flowScannerBtn.title = "Open Flow Scanner for static analysis";
    flowScannerBtn.className = currentUrl.includes("sandbox") ? "flowScannerBtnSandbox" : "flowScannerBtnProd";
    
    // Add styles for the button
    const buttonStyle = document.createElement("style");
    buttonStyle.textContent = `
      .flowScannerBtnSandbox, .flowScannerBtnProd {
      ...
      .flowScannerBtnSandbox:hover, .flowScannerBtnProd:hover {
        background: linear-gradient(135deg, #005fb2 0%, #004a8c 100%);
        transform: translateY(-1px);
        box-shadow: 0 4px 8px rgba(0, 0, 0, 0.15);
      }
      .flowScannerBtnSandbox:active, .flowScannerBtnProd:active {
    ...
    flowScannerBtn.addEventListener("click", () => {
      openFlowScanner(currentUrl);
    });
    
    // Append the button to the header
    headerFlow.appendChild(flowScannerBtn);
  }
